...
if (!$SESSION->IsLoggedIn()) {
    $Return_URL = $CONFIG->URL_ROOT . '/?pag=cadastro';
    header("Location: $Return_URL");
    unset($Return_URL);
    exit;
}
//Verifica se � aluno do IFSP
$query = "SELECT RA FROM participantes WHERE cpf='{$USER->getCpf()}'";
...
if (isset($_POST['alterar_dados'])):
    ...
    if ($aluno) {
        $query = "SELECT * FROM participantes WHERE RA='$prontuario' AND cpf<>'{$USER->getCpf()}'";
...
//Escapa as vari�veis
    ...
    if ($dadosOK) {
        ...
        $query = "UPDATE participantes SET email='$email', confirmado='0', cod_confirmacao='$codigo' WHERE cpf='{$USER->getCpf()}'";
...
if (isset($_POST['senha'])):
    ...
    //Se dados OK insere no banco de dados
    if ($dadosOK) {
        $query = "UPDATE participantes SET senha='" . md5($senha) . "' WHERE cpf='{$USER->getCpf()}'";
...
$query = "SELECT * FROM participantes WHERE cpf='{$USER->getCpf()}'";
...
<form class="forms" name="alt_senha" method="post" action="<? echo $Esta_Pagina; ?>">
    ...
    <label for="csenha">Confirma nova senha:</label>
    <input type="password" name="csenha" id="csenha" maxlength="30"/>
    <br/><br/>
